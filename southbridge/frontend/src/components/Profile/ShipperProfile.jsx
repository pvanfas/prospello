...
import { User, Building, Phone, Mail, Edit2, LogOut, Loader2, Shield, CheckCircle, Clock, XCircle, Hash, FileText, AlertCircle, Upload, Trash2, Award, Download, Eye, X as CloseIcon, Wallet, TrendingUp, CreditCard, ChevronRight } from 'lucide-react';
...
const ShipperProfile = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        setLoading(true);
        const response = await profileAPI.getShipperProfile();
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const response = await profileAPI.updateShipperProfile(editForm);
      setProfileData(response.data);
      setIsEditing(false);
    } catch (err) {
