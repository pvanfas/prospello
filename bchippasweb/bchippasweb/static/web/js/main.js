(function ($) {
    ...
    function setupSwiper() {
        function bindScrollDrivenSwiper(swiperItem, swiperObj) {
            if (!swiperItem || swiperItem.getAttribute('data-scroll-driven') !== '1' || swiperItem.dataset.scrollDrivenBound === '1') {
                return;
            }

            swiperItem.dataset.scrollDrivenBound = '1';
            swiperItem.addEventListener('wheel', function (event) {
                swiperObj = swiperObj || swiperItem.swiper;
                if (!swiperObj || swiperObj.destroyed) {
                    return;
                }

                var rect = swiperItem.getBoundingClientRect();
                var isVisible = rect.top < window.innerHeight && rect.bottom > 0;
                if (!isVisible || Math.abs(event.deltaY) < 2) {
                    return;
                }

                var isForward = event.deltaY > 0;
                var canSlideForward = isForward && !swiperObj.isEnd;
                var canSlideBackward = !isForward && !swiperObj.isBeginning;

                if (!canSlideForward && !canSlideBackward) {
                    return;
                }

                event.preventDefault();
                if (swiperObj.animating) {
                    return;
                }
            ...
            if (typeof (sliderOptions) !== 'undefined' && sliderOptions !== null) {
                ...
                if (typeof Swiper === 'function') {
                    _this.imagesLoaded(function () {
                        ...
                        bindScrollDrivenSwiper(swiperItem, swiperObj);
                    });
                }
            }
        });
